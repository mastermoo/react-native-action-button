...
export default class ActionButton extends Component {
  ...
  componentDidUpdate(nextProps) { // Wring fixed componentWillReceiveProps => componentDidUpdate (for React Native v 0.64.3)
  ...
  animateButton(animate = true) {
    ...
    if (animate) {
      Animated.spring(this.anim, { 
        toValue: 1,  
        useNativeDriver: false // Warining Fixed  
      }).start();
    } else {
  ...
  reset(animate = true) {
    ...
    if (animate) {
      Animated.spring(this.anim, { 
        toValue: 0,  
        useNativeDriver: false // Warning Fixed 
      }).start();
    } else {
